...
@onready var player_info = $"../CanvasLayer/UI/PlayerInfoContainer/PlayerInfo"
...
func calculate_tiles_for_movement(active, character):
	...
	if active:
		...
		tiles_for_movement = map.tiles#.filter(func(tile): return tile.health != HealthType.DESTROYED and tile.health == HealthType.INDESTRUCTIBLE)
...
func is_tile_adjacent(tile, origin_tile, check_for_movement):
	if check_for_movement and (tile.health == HealthType.DESTROYED or tile.health == HealthType.INDESTRUCTIBLE):
		return false
...
func calculate_tiles_for_action(active, character):
	...
	if active:
		...
		tiles_for_action = map.tiles#.filter(func(tile): return tile.health != HealthType.DESTROYED and tile.health == HealthType.INDESTRUCTIBLE)
...
func calculate_first_occupied_tile_for_action_direction_line(character, target_tile_coords):
	if character.action_direction == ActionDirection.HORIZONTAL_LINE:
		...
		var occupied_tiles_in_line = tiles_in_line.filter(func(tile_in_line): return tile_in_line.health == HealthType.INDESTRUCTIBLE or tile_in_line.player or tile_in_line.enemy or tile_in_line.civilian)
...
func _on_player_hovered(player):
	if selected_player and selected_player != player:
		return
	
	if player.current_phase == PhaseType.MOVE:
		var tiles_for_movement = calculate_tiles_for_movement(player.is_hovered, player)
		for tile_for_movement in tiles_for_movement:
			tile_for_movement.toggle_player_hovered(player.is_hovered)
	elif player.current_phase == PhaseType.ACTION:
		var tiles_for_action = calculate_tiles_for_action(player.is_hovered, player)
		for tile_for_action in tiles_for_action:
			tile_for_action.toggle_player_hovered(player.is_hovered)
...
func _on_player_clicked(player):
	...
	if player.is_clicked:
		selected_player = player
		
		# UI
		player_info.text = 'MOVES: ' + str(selected_player.moves_made_current_turn) + '/' + str(selected_player.moves_per_turn) + '\n'
		player_info.text += 'ACTIONS: ' + str(selected_player.actions_made_current_turn) + '/' + str(selected_player.actions_per_turn)
	...
	if player.current_phase == PhaseType.MOVE:
		var tiles_for_movement = calculate_tiles_for_movement(player.is_clicked, player)
		for tile_for_movement in tiles_for_movement:
			tile_for_movement.toggle_player_clicked(player.is_clicked)
	elif player.current_phase == PhaseType.ACTION:
		var tiles_for_action = calculate_tiles_for_action(player.is_clicked, player)
		for tile_for_action in tiles_for_action:
			tile_for_action.toggle_player_clicked(player.is_clicked)
...
func _on_character_action_pull_front(character, origin_tile_coords):
	...
	if target_tiles.is_empty():
		print(str(character.tile.coords) + ' ' + character.name + ': cannot be pulled front')
	else:
		await character.move([target_tiles.front()], true)
